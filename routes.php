...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_route'])) {
    ...
    $checkStmt = $conn->prepare("SELECT RouteID FROM routes WHERE RouteName = ?");
    ...
    // Check start and end point are not the same
    elseif (strtolower($start) == strtolower($end)) {
        $error = "Start point and end point cannot be the same!";
    }
    // Everything fine — save to database
    else {
        $insertStmt = $conn->prepare("INSERT INTO routes (RouteName, StartPoint, EndPoint) VALUES (?, ?, ?)");
        $insertStmt->bind_param('sss', $name, $start, $end);
        $insertStmt->execute();
        $insertStmt->close();
...
if (isset($_GET['delete'])) {
    ...
    $deleteStmt = $conn->prepare("DELETE FROM routes WHERE RouteID = ?");
...
$result = $conn->query("SELECT * FROM routes ORDER BY RouteID DESC");
?>
...
<head>
<meta charset="UTF-8">
<title>Routes - Megacity Taxi</title>
<style>
...
<div class="sidebar">
    ...
    <nav>
        ...
        <a href="routes.php" class="active">🗺️ Routes</a>
...
<div class="main">
    ...
    <div class="content">
        ...
        <div class="form-card">
            <div class="form-title">➕ Add New Route</div>
            ...
            <form method="POST">
                <div class="form-grid">
                    <div>
                        ...
                        <input type="text" name="route_name" placeholder="e.g. Durban CBD - Umlazi" required>
                    </div>
                    <div>
                        <label>Start Point</label>
                        <input type="text" name="start_point" placeholder="e.g. Durban CBD" required>
                    </div>
                    <div>
                        <label>End Point</label>
                        <input type="text" name="end_point" placeholder="e.g. Umlazi" required>
                    </div>
                </div>
                <button type="submit" name="add_route" class="btn btn-primary">Save Route</button>
            </form>
        </div>
        <?php } ?>
            ...
            <table>
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                        ...
                        <td>{$row['EndPoint']}</td>";
                    if ($user['Role'] == 'admin') {
                        echo "<td><a href='routes.php?delete={$row['RouteID']}' class='btn btn-danger' onclick='return confirm(\"Delete this route?\")'>Delete</a></td>";
                    }
                    echo "</tr>";
                    $i++;
                }
